...
A partir de los datos obtenidos, podemos calcular la normalidad de los cuatro atributos numéricos: **age**, **fare**, **status** y **family**. Para ello utilizaremos el test de Saphiro-Wilk:
```{r echo=TRUE, message=FALSE, warning=FALSE}
shapiro.test(ds$age)
shapiro.test(ds$fare)
shapiro.test(ds$status)
...
Seguidamente, pasamos a estudiar la homogeneidad de varianzas mediante la aplicación de un test de Fligner-Killeen. En este caso, estudiaremos la homogeneidad de las cuatro variables anteriormente mencionadas respecto a la variable survived.
```{r echo=TRUE, message=FALSE, warning=FALSE}
ds$survived = as.numeric(ds$survived)
fligner.test(survived ~ age, data = ds)
fligner.test(survived ~ fare, data = ds)
fligner.test(survived ~ status, data = ds)
fligner.test(survived ~ family, data = ds)
```
En el siguiente test, la hipótesis nula consiste en que ambas varianzas son iguales. Por lo tanto en nuestro caso XXX
